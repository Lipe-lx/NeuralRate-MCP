...
- When redeploying contracts or updating secrets, always sync secrets locally first via `npm run cf:worker:secrets:sync` and `npm run cf:executor:secrets:sync` before pushing to GitHub to trigger the CI/CD pipeline (ensuring production secrets are updated on Cloudflare).
